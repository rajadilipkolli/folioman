...
export const mutations = mutationTree(state, {
  UPDATE_PORTFOLIOS: (state: MutualfundsState, newPortfolios: Array<MFPortfolio>) =>
    (state.portfolios = newPortfolios),
  UPDATE_CURRENT_PORTFOLIO: (state: MutualfundsState, newPortfolio: MFPortfolio) =>
  ...
  UPDATE_SUMMARY: (state: MutualfundsState, newSummary: Summary) => (state.summary = newSummary),
})
    ...
    async updateSchemes({ commit, dispatch, getters }, force = false) {
      ...
      try {
        const { data } = await this.$axios.get(
          '/api/mutualfunds/portfolio/' + getters.currentPortfolio.id + '/summary/',
        )
